...
double J(matrix A, matrix X, matrix Y, double lambda=0)
{
    double result = 0, result_reg=0;
    int m = X.getrows();
    ...
    result_reg=(lambda/(2*m))*A(0)*A(0);

    return -(1.0/m)*result+result_reg;
}

// градієнтний спуск
matrix grad_descent(matrix X, matrix Y, string str, double alpha=0.01, int epochs = 10, 
    double lambda=0.05, double epsilon=0.0001, bool out=false)
{
    ...
    do
    {
        ...
        A = A - (1.0/m) * (alpha*grad + lambda*A);
        A[A.getrows()-1] = A[A.getrows()-1] + (lambda/m)*A[A.getrows()-1];
        error_end = J(A, X, Y, lambda);
...
int main()
{
    ifstream f("bank_cpp.txt"); // спроба відкриття файлу
    if(!f) 
    {
        cerr<<"Неможливо відкрити файл bank_cpp.txt"<<endl;
        return 1;
    }
    ...
    {
        cerr<<"Помилка читання з файлу bank_cpp.txt"<<endl;
        return 2;
    }
    ...
    //matrix grad_descent(matrix X, matrix Y, string str, double alpha=0.01, int epochs = 10, 
    //double lambda=0.05, double epsilon=0.0001, bool out=false)
    matrix A_g=grad_descent(X_n, Y, "bank_weights.txt", 0.5, 50, lambda, 0.0001,  true);
